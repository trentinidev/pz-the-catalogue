...
Only **340** pieces of furniture are items the game ships. Everything else in Knox
County — every sofa, bed, bookcase, counter and fridge — is a **tile**, and picking one
up produces a `Base.Moveable` with the sprite name written into the instance. One
fullType for all of them. So the price table could say exactly one thing about the lot,
and it said **five dollars**: the same for a china cabinet and a road cone. The
catalogue also listed that blank `Moveable` for sale, and buying it handed over a
moveable with no sprite in it — an item that cannot be placed, cannot be identified and
cannot be given back.

### Added
- **1,119 pieces of furniture in the catalogue**, generated by `sh tools/gen_furniture.sh`
  from the game's own `newtiledefinitions.tiles.txt`. Sofas, beds, wardrobes, shop
  counters, industrial fridges, rugs, curtains, posters, plants, road cones.

  The fullType is `Moveables.<sprite>`, which the engine's own `InventoryItemFactory`
  reads directly — so a sprite name is a complete address for a piece of furniture, and
  one bought from the catalogue arrives as a **placeable moveable**, not a token.

  Generated offline rather than swept from `IsoSpriteManager` at load, which is the same
  trade the price table makes: thousands of sprite reads on the first open of the buy
  window, for data that only changes when the game does.

- **Sell furniture where it stands.** Right-click a piece and the catalogue quotes a
  price on the menu. What it does **not** skip is the work: it queues *vanilla's* pickup
  action, so a piece that needs a screwdriver still needs one, a piece above your
  carpentry is still refused, and a double bed still comes up as one piece. The money is
  the shortcut; the labour is not.

  Grey rather than hidden when a piece cannot be taken, with a tooltip — "the catalogue
  does not buy wardrobes" and "this one is bolted down and you have no screwdriver" are
  different facts and the player deserves to know which they are looking at.

  The world entry appears only while the catalogue is on you -- the same sandbox switch
  that governs the window. Ordinary house floors are moveables, so without that gate this
  would put a line on every right-click in Knox County whether the player had ever
  crafted a catalogue or not.

  More than one moveable on a square goes into a submenu. A rug, the table on it, the
  lamp behind and the poster beside it are four pieces, and four more lines on a menu
  vanilla has already filled is how a mod becomes the reason nobody can find
  "Open window".

### Changed
- **A price per category, not per piece.** Every other price in this mod comes from the
  study, which priced 5,092 items one at a time; the study never saw these, because they
  are not items. Rather than invent 1,119 numbers, each piece carries a category and
  `TC_Furniture.lua` carries the money — twenty numbers you can read in one screen and
  tune.

  The figures are **anchored, not guessed**: where the study has an opinion about a piece
  of furniture the game does ship as an item, that opinion sets the category. Chair $20,
  table $31, lamp $7, fridge $175, desktop computer $355. `Refrigeration` and `Computer`
  are split out of their obvious parents for exactly that reason — one number cannot hold
  both a $175 fridge and an $8 oven without lying about one of them.

- **The 340 keep their study prices.** A sprite that names a `CustomItem` is one of them,
  so the catalogue quotes the same number for the chair on the shelf and the chair in the
  room, and this feature does not quietly re-price what already worked.

- **Furniture from another mod is still bought**, at the Misc rate. Unlisted furniture is
  still furniture, and refusing it would be a worse answer than paying the going rate for
  an unknown piece.

- **Deliveries build the item instead of naming it.** `AddWorldInventoryItem` and
  `AddItem` were handed a fullType string, which cannot work for furniture — and
  `TC.createItem` is also where the **sprite-grid anchor** is resolved, which is the
  difference between delivering a bed and delivering half of one. A wardrobe weighs more
  than the largest crate holds, so it arrives standing beside the boxes.

### Removed
- `Base.Moveable` from the shelf. Not furniture — the *absence* of furniture.

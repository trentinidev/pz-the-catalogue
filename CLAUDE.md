...
## Layout
    ...
    42/media/lua/client/TheCatalogue/   seven windows, money, four context menus, actions
...
`TC_FurnitureTable.lua` is the same shape at 103 KB: 1,119 rows of `{ s, n, c, w }`
generated by `sh tools/gen_furniture.sh` from the game's own
`media/newtiledefinitions.tiles.txt` (6.8 MB, in the install, deliberately not vendored).
Grep it for a sprite; never read it whole. The MONEY is not in it -- prices are per
category in `TC_Furniture.lua`, twenty numbers, and that is the file to edit.
...
- **A tile is identified by its SPRITE NAME and nothing else.** The four vanilla ATMs
  ...
  length-prefixed PNG.
- **Furniture is a TILE, not an item.** Only 340 pieces ship as items; picking up
  anything else gives a `Base.Moveable` with the sprite on the INSTANCE, so `getFullType`
  says nothing about what it is. `TC.moveableSprite` is the question to ask, and it has
  to be asked BEFORE the exclusion list, since `Base.Moveable` is on it. The engine's
  `InventoryItemFactory` special-cases the fullType prefix `Moveables.`, which is what
  lets a sprite name be bought, sold and delivered like any other id.
- **`instanceof` before any Java getter that is not on `InventoryItem`.** Calling a
  method that does not exist is a hard Kahlua error, not a nil -- `getUsedDelta` went
  away in B42 and took every drainable sale with it. `instanceof` is a Java global and
  is safe to point at an object of a class Lua has never been shown; a method call is not.
